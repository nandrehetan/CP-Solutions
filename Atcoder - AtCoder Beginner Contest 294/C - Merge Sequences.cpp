...
void hetu()
{
    ...
    vector<ll>b(m);
    vector<ll>c;
    rep(i,0,n){cin>>a[i];c.push_back(a[i]);}
    rep(i,0,m){cin>>b[i];c.push_back(b[i]);}
    sort(all(c));
    rep(i,0,n){
        ll ok = lower_bound(c.begin(),c.end(),a[i])-c.begin();
        cout<<ok+1<<" ";
    }cout<<"\n";
    rep(i,0,m){
        ll ok1 = lower_bound(c.begin(),c.end(),b[i])-c.begin();
        cout<<ok1+1<<" ";
    }cout<<"\n";
