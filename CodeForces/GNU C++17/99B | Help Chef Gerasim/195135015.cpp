...
void hetu()
{
    ...
    for(int i= 0 ;i < n /2;i++)
    {
        ...
        {
            ans1 = v[i].second,ans2 = v[n-i-1].second;
            ans3 = v[n-i-1].first - v[i].first;
            if((v[n-i-1].first - v[i].first)&1)ok = false;
            v[i].first = v[i].first + ans3 / 2;
            v[n-i-1].first = v[i].first;
            cnt++;
        }
    }
    set<int> k;
    ...
    cout << ans3 / 2 << " ml. from cup #"<<ans1 + 1 <<" to cup #"<<ans2 + 1<<".";
}
