...
bool recur(ll index, ll used, ll value)
{
    if (value < 0)
    {
        return false;
    ...
    if (dp[index][used][value] != -1)
    {
        return dp[index][used][value];
    }

    if (recur(index-1,0,value))
    {
        return dp[index][used][value] = true;
    }
    if (used < vp[index].second)
    {
        if (recur(index,used+1,value-vp[index].first))
        {
            return  dp[index][used][value] = true;
        }
    }
    return dp[index][used][value] = false;
}
/*/-----------------------------Code begins----------------------------------/*/
